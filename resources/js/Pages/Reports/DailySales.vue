...
<template>
    <div class="flex flex-col h-full overflow-hidden">
        ...
        <!-- Filters -->
        <div class="px-6 py-3 border-b border-slate-700 flex flex-wrap items-end gap-4 flex-shrink-0 bg-slate-800/50">
            <div v-if="canSeeAll">
                <label class="block text-xs text-slate-400 mb-1">Cashier / Staff</label>
                <select v-model="userId" class="bg-slate-700 text-white px-3 py-2 rounded-lg text-sm outline-none border border-slate-600 focus:border-emerald-500 transition">
            ...
            <button @click="doFilter" class="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm rounded-lg transition font-medium shadow-lg shadow-emerald-900/20">Filter</button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-slate-800 border border-slate-700 rounded-xl p-5">
                    ...
                    <p class="text-2xl font-bold text-white font-mono">{{ summary.total_sales }}</p>
                </div>
                <div class="bg-slate-800 border border-slate-700 rounded-xl p-5">
                    <p class="text-xs text-slate-400 mb-1">Total Revenue</p>
                    <p class="text-2xl font-bold text-emerald-400 font-mono">{{ format(summary.total_revenue) }}</p>
                </div>
                <div class="bg-slate-800 border border-slate-700 rounded-xl p-5">
                    <p class="text-xs text-slate-400 mb-1">Discounts Given</p>
                    <p class="text-2xl font-bold text-red-400 font-mono">{{ format(summary.total_discount) }}</p>
                </div>
                <div class="bg-slate-800 border border-slate-700 rounded-xl p-5">
                    <p class="text-xs text-slate-400 mb-1">Gross Profit</p>
                    <p class="text-2xl font-bold text-blue-400 font-mono">{{ format(summary.total_profit) }}</p>
                </div>
            </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-700/50">
                        <tr v-for="sale in sales.data" :key="sale.id" class="hover:bg-slate-700/30 transition">
                            <td class="px-5 py-3 text-slate-400 text-xs font-mono">{{ dayjs(sale.created_at).format('DD MMM YYYY HH:mm') }}</td>
                            ...
                            <td class="px-5 py-3 text-slate-300 text-xs">{{ sale.customer?.name ?? 'Walk-in Customer' }}</td>
                            ...
                            <td class="px-5 py-3 text-slate-400 text-right capitalize text-xs font-medium">{{ sale.payment_method }}</td>
                        </tr>
                        <tr v-if="!sales.data.length">
                            <td colspan="6" class="px-5 py-12 text-center text-slate-500">No sales recorded matching current filters.</td>
                        </tr>
                    </tbody>
                </table>
