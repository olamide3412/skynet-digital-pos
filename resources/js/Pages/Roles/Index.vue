...
<template>
    <div class="flex flex-col h-full overflow-hidden">
        <div class="flex items-center gap-3 px-6 py-4 border-b border-slate-700 bg-slate-800 flex-shrink-0">
            <Link :href="route('pos.users.index')" class="text-slate-400 hover:text-white transition">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            </Link>
            <h1 class="text-lg font-bold text-white flex-1">System Roles</h1>
        ...
        <div class="flex-1 overflow-y-auto p-6 flex justify-center">
            <div class="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden w-full max-w-3xl h-fit">
                <table class="w-full text-sm">
                    <thead class="bg-slate-700/50 border-b border-slate-700">
                        <tr>
                            <th class="text-left px-5 py-3 text-slate-400 font-medium w-1/3">Role Name</th>
                            <th class="text-left px-5 py-3 text-slate-400 font-medium">Description</th>
                            <th class="text-center px-5 py-3 text-slate-400 font-medium w-24">Users</th>
                            <th class="px-5 py-3 w-28"></th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-700/50">
                        <tr v-for="role in roles" :key="role.id" class="hover:bg-slate-700/30 transition">
                            <td class="px-5 py-3 text-white font-bold tracking-wide uppercase text-xs">{{ role.name }}</td>
                            <td class="px-5 py-3 text-slate-400">{{ role.description || '—' }}</td>
                            <td class="px-5 py-3 text-center">
                                <span class="bg-slate-700 px-2 py-0.5 rounded text-emerald-400 font-medium">{{ role.users_count }}</span>
                            </td>
                            <td class="px-5 py-3 text-right">
                                <div class="flex items-center justify-end gap-2 text-xs">
                                    <button @click="openEditModal(role)" class="text-blue-400 hover:text-blue-300 transition">Edit</button>
                                    <button v-if="!['admin','cashier'].includes(role.name.toLowerCase())" @click="destroy(role)" class="text-red-400 hover:text-red-300 transition">Delete</button>
                                </div>
                            </td>
                        </tr>
                        <tr v-if="!roles.length">
                            <td colspan="4" class="px-5 py-12 text-center text-slate-500">No roles configured.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <dialog id="role-modal" class="bg-transparent backdrop:bg-slate-900/80 p-0 m-auto rounded-xl shadow-2xl backdrop-blur-sm open:animate-in open:zoom-in-95 w-full max-w-sm">
            <div class="bg-slate-800 border border-slate-700 overflow-hidden rounded-xl">
                <div class="px-6 py-4 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
                    <h3 class="font-bold text-white">{{ isEditing ? 'Edit Role' : 'New Role' }}</h3>
                ...
                <div class="p-6">
                    <form @submit.prevent="submit" class="space-y-4">
                        <div>
                            <label class="block text-xs text-slate-400 mb-1">Role Name *</label>
                        ...
                        <div>
                            ...
                            <textarea v-model="form.description" rows="3" class="w-full bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-600 outline-none text-sm focus:border-emerald-500 transition resize-none"></textarea>
                        </div>
