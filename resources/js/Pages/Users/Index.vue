...
<template>
    <div class="flex flex-col h-full overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-700 bg-slate-800 flex-shrink-0">
            <h1 class="text-lg font-bold text-white">System Users & Staff Permissions</h1>
            <div class="flex gap-2">
                ...
                <button @click="openCreateModal" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm rounded-lg transition font-medium flex items-center gap-1.5 shadow-lg shadow-emerald-600/20">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-slate-700/50 border-b border-slate-700">
                        <tr>
                            ...
                            <th class="text-left px-5 py-3 text-slate-400 font-medium">Role</th>
                            <th class="text-left px-5 py-3 text-slate-400 font-medium">Granted Permissions</th>
                    ...
                    <tbody class="divide-y divide-slate-700/50">
                        <tr v-for="user in users.data" :key="user.id" class="hover:bg-slate-700/30 transition">
                            <td class="px-5 py-3 text-white font-medium">
                                {{ user.name }}
                                <div v-if="user.email" class="text-xs text-slate-400 font-normal">{{ user.email }}</div>
                            </td>
                            <td class="px-5 py-3 text-slate-300 font-mono text-xs">{{ user.username }}</td>
                            <td class="px-5 py-3">
                                <div class="flex flex-wrap gap-1">
                                    <span v-for="role in user.roles" :key="role.id"
                                        :class="role.name === 'branch-admin' ? 'bg-indigo-500/20 text-indigo-300 border-indigo-500/40' : 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40'"
                            ...
                            <td class="px-5 py-3">
                                <span v-if="user.roles?.some(r => r.name === 'branch-admin')" class="text-xs text-indigo-300 font-medium bg-indigo-500/10 px-2 py-0.5 rounded border border-indigo-500/20">
                                    Full Access (All Permissions)
                                </span>
                                <span v-else-if="user.permissions?.length" class="text-xs text-emerald-300 font-medium bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                            ...
                            <td class="px-5 py-3">
                                <span class="px-2 py-0.5 text-xs rounded-full" :class="user.is_active ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'">
                                    {{ user.is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-5 py-3 text-right">
                                <div class="flex items-center justify-end gap-2 text-xs">
                                    <button @click="openEditModal(user)" class="text-blue-400 hover:text-blue-300 transition font-medium">Edit</button>
                                    ...
                                    <button v-if="$page.props.auth?.permissions?.canResetPassword" @click="openResetModal(user)" class="text-purple-400 hover:text-purple-300 transition font-medium">Reset Pass</button>
                                </div>
                            </td>
                        </tr>
                        <tr v-if="!users.data.length">
                            <td colspan="6" class="px-5 py-12 text-center text-slate-500">No users found.</td>
                        </tr>
                    </tbody>
                </table>
        ...
        <dialog id="user-modal" class="bg-transparent backdrop:bg-slate-900/80 p-0 m-auto rounded-xl shadow-2xl backdrop-blur-sm open:animate-in open:zoom-in-95 w-full max-w-2xl">
            <div class="bg-slate-800 border border-slate-700 overflow-hidden rounded-xl flex flex-col max-h-[90vh]">
                <div class="px-6 py-4 border-b border-slate-700 flex justify-between items-center bg-slate-800/50 flex-shrink-0">
                    <div>
                        <h3 class="font-bold text-white">{{ isEditing ? 'Edit User & Permissions' : 'Create New Staff User' }}</h3>
                        <p class="text-xs text-slate-400">Assign role and grant specific feature permissions</p>
                    </div>
                    ...
                    <form @submit.prevent="submit" class="space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select v-model="form.role" required class="w-full bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-600 outline-none text-sm focus:border-emerald-500 transition">
                        ...
                        <div class="border-t border-slate-700 pt-4 space-y-3">
                            <div class="flex items-center justify-between">
                                <div>
                                    <h4 class="text-xs uppercase font-bold text-emerald-400 tracking-wider">Feature Permissions</h4>
                                    <p class="text-xs text-slate-400">Toggle individual features for this user</p>
                                </div>
                                <div class="flex gap-2">
                                    <button type="button" @click="selectAll" class="text-xs text-emerald-400 hover:text-emerald-300 px-2 py-1 bg-slate-700 rounded border border-slate-600">Select All</button>
                                    <button type="button" @click="deselectAll" class="text-xs text-slate-400 hover:text-slate-200 px-2 py-1 bg-slate-700 rounded border border-slate-600">Deselect All</button>
                                </div>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto p-2 bg-slate-900/60 rounded-lg border border-slate-700">
                                <label v-for="perm in all_permissions" :key="perm.name"
                                    class="flex items-center gap-2.5 p-2 rounded cursor-pointer text-xs transition"
                                    :class="form.permissions.includes(perm.name) ? 'bg-emerald-500/10 text-white font-medium' : 'text-slate-400 hover:bg-slate-700/50'"
                                >
                                    <input
                                        type="checkbox"
                                        :value="perm.name"
                        ...
                        <div class="pt-4 flex gap-3 flex-shrink-0">
                            ...
                            <button type="submit" :disabled="form.processing" class="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-sm font-medium transition disabled:opacity-40 shadow-lg shadow-emerald-600/20">
        ...
        <dialog id="reset-modal" class="bg-transparent backdrop:bg-slate-900/80 p-0 m-auto rounded-xl shadow-2xl backdrop-blur-sm w-full max-w-md">
            <div class="bg-slate-800 border border-slate-700 overflow-hidden rounded-xl">
                <div class="px-6 py-4 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
                    <h3 class="font-bold text-white">Reset Password for {{ userToReset?.username }}</h3>
                    <form method="dialog"><button class="text-slate-400 hover:text-white transition"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button></form>
                </div>
                <form @submit.prevent="submitResetPassword" class="p-6 space-y-4">
                    <div>
                        ...
                        <input v-model="resetForm.password" type="password" required minlength="8" class="w-full bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-600 outline-none text-sm focus:border-purple-500 transition" />
                        <p v-if="resetForm.errors.password" class="text-red-400 text-xs mt-1">{{ resetForm.errors.password }}</p>
                    </div>
                    <div>
                        <label class="block text-xs text-slate-400 mb-1">Confirm New Password *</label>
                        <input v-model="resetForm.password_confirmation" type="password" required minlength="8" class="w-full bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-600 outline-none text-sm focus:border-purple-500 transition" />
                    </div>
                    <div class="pt-2 flex gap-3">
                        <form method="dialog" class="flex-1">
                            <button class="w-full py-2 bg-slate-700 text-slate-300 rounded-lg text-sm hover:bg-slate-600 transition">Cancel</button>
                        </form>
                        <button type="submit" :disabled="resetForm.processing" class="flex-1 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm font-bold transition disabled:opacity-40">
