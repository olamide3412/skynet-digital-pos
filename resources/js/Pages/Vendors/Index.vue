...
<template>
    <div class="flex flex-col h-full overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-700 bg-slate-800 flex-shrink-0">
            <h1 class="text-lg font-bold text-white">Vendors & Suppliers</h1>
        ...
        <div class="px-6 py-3 border-b border-slate-700 flex gap-2 flex-shrink-0">
            <input v-model="search" @keydown.enter="doSearch" type="text" placeholder="Search by name or company…" class="flex-1 max-w-md bg-slate-700 text-white placeholder-slate-400 px-3 py-2 rounded-lg text-sm outline-none border border-slate-600 focus:border-emerald-500 transition" />
        ...
        <div class="flex-1 overflow-y-auto px-6 py-4">
            <div class="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
                <table class="w-full text-sm">
                    <thead class="bg-slate-700/50 border-b border-slate-700">
                        <tr>
                            ...
                            <th class="text-left px-4 py-3 text-slate-400 font-medium">Company</th>
                            <th class="text-left px-4 py-3 text-slate-400 font-medium">Phone</th>
                            <th class="text-center px-4 py-3 text-slate-400 font-medium">Orders</th>
                            <th class="text-left px-4 py-3 text-slate-400 font-medium">Status</th>
                            <th class="px-4 py-3"></th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-700/50">
                        <tr v-for="vendor in vendors.data" :key="vendor.id" class="hover:bg-slate-700/30 transition">
                            <td class="px-4 py-3 text-white font-medium">{{ vendor.name }}</td>
                            <td class="px-4 py-3 text-slate-400">{{ vendor.company_name ?? '—' }}</td>
                            <td class="px-4 py-3 text-slate-400">{{ vendor.phone }}</td>
                            <td class="px-4 py-3 text-center text-emerald-400 font-medium">{{ vendor.purchase_orders_count }}</td>
                            <td class="px-4 py-3">
                                <span class="px-2 py-0.5 text-xs rounded-full" :class="vendor.status === 'Active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'">
                                    {{ vendor.status }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button @click="openEditModal(vendor)" class="text-xs text-blue-400 hover:text-blue-300 transition">Edit</button>
                                    <button @click="destroy(vendor)" class="text-xs text-red-400 hover:text-red-300 transition">Delete</button>
                                </div>
                            </td>
                        </tr>
                ...
                <div v-if="vendors.last_page > 1" class="flex items-center justify-between px-4 py-3 border-t border-slate-700 text-xs text-slate-400">
                    <span>{{ vendors.from }}–{{ vendors.to }} of {{ vendors.total }}</span>
                    <div class="flex gap-1">
                        <Link v-if="vendors.prev_page_url" :href="vendors.prev_page_url" class="px-3 py-1 rounded bg-slate-700 hover:bg-slate-600 transition">Prev</Link>
                        <Link v-if="vendors.next_page_url" :href="vendors.next_page_url" class="px-3 py-1 rounded bg-slate-700 hover:bg-slate-600 transition">Next</Link>
                    </div>
                </div>
            </div>
        </div>
        ...
        <dialog id="vendor-modal" class="bg-transparent backdrop:bg-slate-900/80 p-0 m-auto rounded-xl shadow-2xl backdrop-blur-sm open:animate-in open:zoom-in-95 w-full max-w-lg">
            <div class="bg-slate-800 border border-slate-700 overflow-hidden rounded-xl">
                <div class="px-6 py-4 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
                    <h3 class="font-bold text-white">{{ isEditing ? 'Edit Vendor' : 'New Vendor' }}</h3>
                    ...
                    <form @submit.prevent="submit" class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input v-model="form.company_name" type="text" class="w-full bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-600 outline-none text-sm focus:border-emerald-500 transition" />
                            </div>
                            <div>
                                <label class="block text-xs text-slate-400 mb-1">Phone *</label>
                                <input v-model="form.phone" type="text" required class="w-full bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-600 outline-none text-sm focus:border-emerald-500 transition" />
                            </div>
                            <div>
                            ...
                            <div class="col-span-2">
                                ...
                                <input v-model="form.address" type="text" class="w-full bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-600 outline-none text-sm focus:border-emerald-500 transition" />
                            </div>
                            <div class="col-span-2">
