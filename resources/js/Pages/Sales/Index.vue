...
function doFilter() {
    router.get(route('pos.sales.index'), { from: from.value, to: to.value }, { preserveState: true, replace: true })
}
    ...
    <div class="flex flex-col h-full overflow-hidden">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-700 bg-slate-800 flex-shrink-0">
            ...
            <div class="flex items-center gap-2">
                ...
                <button @click="doFilter" class="px-3 py-1.5 bg-slate-700 hover:bg-slate-600 text-white text-xs rounded-lg transition">Filter</button>
            </div>
        </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-slate-700/50 border-b border-slate-700">
                        <tr>
                            ...
                            <th class="text-left px-4 py-3 text-slate-400 font-medium">Cashier</th>
                    ...
                    <tbody class="divide-y divide-slate-700/50">
                        <tr v-for="sale in sales.data" :key="sale.id" class="hover:bg-slate-700/30 transition">
                            ...
                            <td class="px-4 py-3 text-slate-400 text-xs">{{ sale.user?.full_name || sale.user?.name }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-right font-medium text-white">{{ format(sale.final_total) }}</td>
                            <td class="px-4 py-3 text-right text-emerald-400 text-sm">{{ format(sale.profit_made) }}</td>
                        ...
                        <tr v-if="!sales.data.length">
                            <td colspan="8" class="px-4 py-12 text-center text-slate-500">No sales in this period.</td>
                        </tr>
                    </tbody>
                </table>
                <!-- Pagination -->
