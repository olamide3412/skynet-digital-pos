    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
        <div class="bg-slate-800 rounded-xl shadow-2xl w-full max-w-sm border border-slate-700">
            <div class="px-5 py-4 border-b border-slate-700 flex items-center justify-between">
                <h3 class="font-bold text-white">Hold Cart</h3>
                <button @click="emit('close')" class="text-slate-400 hover:text-white transition">✕</button>
            </div>
            <div class="px-5 py-4 space-y-3">
                <p class="text-slate-400 text-sm">{{ cart.items.length }} item(s) will be saved. Add a name to find it easily.</p>
                <input
                    v-model="holdName"
                    type="text"
                    placeholder="Hold name (optional)"
                    class="w-full bg-slate-700 text-white px-3 py-2 rounded-lg outline-none border border-slate-600 focus:border-amber-500 text-sm transition"
                />
                <p v-if="error" class="text-red-400 text-sm">{{ error }}</p>
            </div>
            <div class="px-5 pb-5 flex gap-2">
                <button @click="emit('close')" class="flex-1 py-2 rounded-lg bg-slate-700 text-slate-300 text-sm transition hover:bg-slate-600">Cancel</button>
